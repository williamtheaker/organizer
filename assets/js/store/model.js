...
export default class Model {
    ...
    bindActionCreators(dispatch) {
        const funcNames = ['saving', 'saved', 'save', 'fetchOne', 'fetchIfNeeded', 'fetchAll', 'update', 'updateAndSave', 'request', 'receive']
    ...
    fetchIfNeeded(id) {
        return (dispatch, getState) => {
            const selector = new ModelSelector(modelCooker(this.name)(getState()))
            if (selector.shouldFetch(id)) {
            ...
            return queuedFetch(url+'?'+urlParams, {credentials: 'include'})
                ...
                .then(json => {
                    ...
                    if (json.next) {
                        ...
                        return dispatch(this.fetchAll({...params, page: nextPage}, this.options))
                    }
                })
        }
    }
